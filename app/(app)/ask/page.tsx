...
export default function AskPage() {
  ...
  return <div className="screen chat-screen"><Header title="Ask Trail" back={() => router.push("/trail")} action={<button className="text-action" onClick={() => router.push("/ask/brief")}>View brief</button>} />
    ...
    {messages.length === 1 && <section className="starter-section"><div className="starter-head"><b>What are you looking for?</b><span>Tell Trail naturally</span></div><div className="starter-list">{starters.map((item) => <button key={item.title} onClick={() => sendMessage(item.prompt)}><i>{item.icon}</i><span><b>{item.title}</b><small>{item.prompt}</small></span><em><IconChevronRight /></em></button>)}</div></section>}
    ...
    <div className="quick-replies"><button onClick={() => sendMessage(trip.areas.length ? `Find stores along ${trip.areas.slice(0, 2).join(" and ")}.` : `Find stores in ${trip.city}.`)}>Along my route</button><button onClick={() => sendMessage("I want hotel bag transfer for anything heavy, fragile or chilled.")}>Hands-free all day</button><button onClick={() => sendMessage(`My gift budget is ${price(wallet.plannedCents, trip.currency)}.`)}>Budget {price(wallet.plannedCents, trip.currency)}</button></div>
