...
import type { BudgetBucket, DataSource, Handling, InquiryStatus, PaymentStatus, PlanStatus, StopStatus, TransferActor, TransferEventType, TransferStatus, TripStatus } from "./types";
...
export type PurchaseRow = { id: string; stop_id: string | null; actual_price_cents: number; quantity: number; bags: number; handling: Handling; currency: string; note: string | null; unplanned_label: string | null; recorded_at: string; voided_at: string | null; void_reason: string | null; updated_at: string };
...
export type StopRow = { id: string; plan_id: string; sequence: number; planned_day: number; status: StopStatus; recipient_id: string | null; product_name: string; store_name: string; store_address: string; area: string; snapshot_price_cents: number; handling: Handling; walk_minutes: number | null; rationale: string; saved: boolean; replaced_stop_id: string | null; source: DataSource; updated_at: string; purchases: PurchaseRow[] | null; store_inquiries: InquiryRow[] | null };

export type StoreRow = { id: string; name: string; address: string; area: string; dropoff_cutoff: string | null; lat: number | null; lng: number | null };
...
export type PaymentRow = { id: string; status: PaymentStatus; amount_cents: number; currency: string; method_brand: string | null; method_last4: string | null; failure_code: string | null; authorized_at: string | null; captured_at: string | null; refunded_at: string | null; created_at: string };
...
export type TransferRow = { id: string; status: TransferStatus; reference_code: string; hotel_name: string; hotel_address: string; bag_count: number; weight_grams: number | null; fee_cents: number; currency: string; eta_start: string | null; eta_end: string | null; dropoff_cutoff_at: string | null; confirmed_at: string | null; delivered_at: string | null; ineligible_reason: string | null; source: DataSource; created_at: string; updated_at: string; dropoff_store: StoreRow | null; bag_transfer_items: TransferItemRow[] | null; transfer_events: TransferEventRow[] | null; payments: PaymentRow[] | null; receipts: ReceiptRow[] | null };

export type TripRow = { id: string; status: TripStatus; country: string; city: string; areas: string[] | null; start_date: string | null; end_date: string | null; hotel_name: string; hotel_address: string; hotel_verified_at: string | null; companions: string; free_time: string; currency: string; updated_at: string; plans: PlanRow[] | null; recipients: RecipientRow[] | null; stops: StopRow[] | null; unplanned_purchases: PurchaseRow[] | null; bag_transfers: TransferRow[] | null };
