...
import { handlingLabel, money, walkLabel } from "../../view";
...
export default function ShopPage() {
  ...
  return <div className="screen shop-screen"><Header title="Shop in store" back={() => router.push("/trail/plan/gifts")} action={<span className={`budget-pill ${over ? "over" : ""}`}>{over ? `$${money(Math.abs(wallet.spendableCents))} over` : `$${money(wallet.spendableCents)} left`}</span>} />
    <h1 className="visually-hidden">Shop in store</h1>
    <section className="shop-summary"><span><small>TODAY’S ROUTE</small><b>{bought.length}/{stops.length} stops bought</b><em>{bagCount} purchased bag{bagCount === 1 ? "" : "s"} · {trip.freeTime} still out</em></span><strong>${money(wallet.spentCents)}<small>spent</small></strong></section>
    {over && <div className="budget-warning" role="status"><b>{currency} ${money(Math.abs(wallet.spendableCents))} over the gift budget</b><span>Edit a purchase, or keep the overage. Nothing is changed for you.</span><button onClick={() => router.push("/trail/plan/budget")}>Review budget</button></div>}
    ...
    <div className="shop-route-line">{stops.map((stop) => <article className={stop.status} key={stop.id}><i>{stop.status === "bought" ? <IconCheck /> : stop.sequence}</i><div><small>{stop.area} · {walkLabel(stop.walkMinutes)}</small><h2>{stop.storeName}</h2><p>Look for {stop.productName}</p><em>{stop.storeAddress} · {handlingLabel[stop.handling]}</em>
      ...
      {stop.status === "bought" && stop.purchase && <button className="purchase-receipt" onClick={() => router.push(`/trail/shop/${stop.id}/record`)}>${money(stop.purchase.actualPriceCents)} total · {stop.purchase.bags} bag{stop.purchase.bags === 1 ? "" : "s"} · {stop.purchase.handling}{stop.purchase.id.startsWith("pending:") ? " · saving…" : ""} · Edit</button>}
