...
export default function TrailDashboard() {
  ...
  return <div className="screen home-screen"><Header action={<button className="avatar" aria-label="Open trips and account" onClick={() => router.push("/trips")}>{(trip.city[0] ?? "T").toUpperCase()}</button>} />
    ...
    {shoppingStarted && <section className="shop-summary"><span><small>TODAY’S ROUTE</small><b>{bought.length}/{stops.length} stops bought</b><em>{bagCount} purchased bag{bagCount === 1 ? "" : "s"} · {trip.freeTime} still out</em></span><strong>{price(wallet.spentCents, trip.currency)}<small>spent</small></strong></section>}
