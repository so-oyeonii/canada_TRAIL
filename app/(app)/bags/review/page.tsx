...
export default function BagReviewPage() {
  ...
  const remedy = (action: Remedy) => {
    ...
    // The same door as Continue, not a shortcut past it. `/bags/pay` calls `confirmTransfer`,
    // which answers 409 `no_bags` when the manifest was never written — so jumping straight
    // there told a traveller who had plainly chosen bags to "select at least one bag first".
    if (action === "approve_flexible") return void next();
