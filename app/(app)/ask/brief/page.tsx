...
export default function BriefPage() {
  ...
  return <div className="screen review-screen"><Header title="Shopping brief" back={() => router.push("/ask")} action={<span className="draft-badge">AI DRAFT</span>} />
    ...
    <section className="budget-editor"><div><span><small>GIFT BUDGET</small><b>{price(toMinor(plan.budget, currency), currency)}</b></span><em>Delivery reserve of {price(wallet.reserveCents, currency)} is held back for your bags</em></div><input type="range" min="40" max="300" step="10" value={plan.budget} aria-label={`Gift budget in ${currency}`} onChange={(e) => updatePlan("budget", Number(e.target.value))} /><div className="range-values"><span>{currencySymbol(currency)}40</span><span>{currencySymbol(currency)}300</span></div></section>
