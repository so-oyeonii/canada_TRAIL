...
export default function BriefPage() {
  ...
  const { trip, plan, updatePlan, routeDirty, estimates, approvePlan } = useApp();
  ...
  return <div className="screen review-screen"><Header title="Shopping brief" back={() => router.push("/ask")} action={<span className="draft-badge">AI DRAFT</span>} />
    ...
    <section className="budget-editor"><div><span><small>GIFT BUDGET</small><b>CAD ${plan.budget}</b></span><em>Bag transfer estimated separately</em></div><input type="range" min="40" max="300" step="10" value={plan.budget} aria-label="Gift budget in Canadian dollars" onChange={(e) => updatePlan("budget", Number(e.target.value))} /><div className="range-values"><span>$40</span><span>$300</span></div></section>
    ...
    <div className="plan-impact"><div><i><IconRoute /></i><span><small>ROUTE</small><b>{estimates.stops} stores · +13 min</b></span></div><div><i><IconBag /></i><span><small>TRANSFER</small><b>{plan.hotelDelivery ? "Hotel-ready stops" : "Carry purchases"}</b></span></div></div>
